...
localIPs=$(ip a | grep inet | grep -v inet6 | grep '/24' | egrep -o $ip_regex | sort | uniq | grep -v '.255$')
echo "my ip is [${localIPs}]"
echo ""
# 转义
localIPsVal=''
for ip in $localIPs; do
  localIPsVal=${localIPsVal}\\\"$ip\\\",
done


hostFields=${localIPsVal}${hostFields}
...
# 删除多余文件
ls | grep -v pem | grep -v config | xargs -i rm -rf {}
